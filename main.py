...
from models.INN import Hinet
#from models.RevealNet import RevealNet
import torchvision.transforms as trans

from models.ResCBAM import CBAM
...
from modules.Unet_common  import gauss_noise
...
parser.add_argument('--decay_round', type=int, default=6,
...
parser.add_argument('--Rnet', default='',
                    help="path to RevealNetImproved (to continue training)")
...
parser.add_argument('--remark', default='main_udh', help='comment')
...
parser.add_argument('--bs_secret', type=int, default=1, help='batch size for ')
...
parser.add_argument('--fake', type=str, default='flower',help='fake folder')
...
def main():
    ...
    global opt, optimizer,  writer, logPath, scheduler,  val_loader, smallestLoss, DATA_DIR
...
#    if opt.hostname == 'workspace.featurize.cn'
#    DATA_DIR = '/hy-tmp/dataset/'
    DATA_DIR = r"F:\Downloads\Universal-Deep-Hiding-main1\Universal-Deep-Hiding-main\dataset"
    assert DATA_DIR
    ...
    if opt.test == '':
        ...
        for epoch in range(opt.epochs):
            ...
            ######################## train ##########################################
            train(train_loader, epoch, Hnet=Hnet, criterion=criterion)

            ####################### validation  #####################################
            val_hloss, val_rloss, val_hdiff, val_rdiff = validation(val_loader, epoch, Hnet=Hnet,  criterion=criterion)
     ...
     # For testing the trained network
        ...
        analysis(test_loader, 0, Hnet=Hnet,  HnetD=HnetD, criterion=criterion)
...
def forward_pass(secret_img, secret_target, cover_img, cover_target, Hnet, criterion, val_cover=0, i_c=None, position=None, Se_two=None):
...
#    if opt.cuda:
    ...
    a=nn.Conv2d(3,3,kernel_size=3, stride=2, padding=1).cuda()
    secret_feature = a(secret_img).cuda()
    #print(f'。。。。。。{secret_feature.shape,secret_feature.shape[1]}')

    secret_attention_model = CBAM(channel = secret_feature.shape[1]).cuda()
    secret_attention = secret_attention_model(secret_feature).cuda()
    #print(f"。。。。。。。。。。。。。。。。。。。{secret_attention.shape}")
    cover_img = cover_img.view(batch_size_cover // opt.num_cover, channel_cover * opt.num_cover, opt.imageSize, opt.imageSize)
    #print(f"cover_img。。。。。。。。{cover_img.shape}")
    ...
    #print(f'........{secret_imgv.shape}')

    secret_attention = F.interpolate(secret_attention,scale_factor = 2)

    # 定义输入图像和缩放倍数
#    input_image = secret_attention  # 输入图像
#   scale_factor = 2  # 缩放倍数
...
#    delta_h, delta_w = new_h - h, new_w - w
    ...
    # 对输入图像进行填充操作
#    padded_image = F.pad(input_image, (0, delta_w, 0, delta_h), mode='replicate')

    # 对填充后的图像进行缩放
#    secret_attention = F.interpolate(padded_image, size=(new_h, new_w), mode="bilinear", align_corners=True)
    #print(f'........{secret_attention.shape}')
    #cover_imgv =F.interpolate(cover_imgv,scale_factor = 0.5)
    #secret_imgv = F.interpolate(secret_imgv,scale_factor = 0.5)
    dwt = common.DWT()
    iwt = common.IWT()
    secret_attention1= dwt(secret_attention)
    secret_imgv =dwt(secret_imgv)
    if opt.cover_dependent:
        H_input = torch.cat((cover_imgv, secret_imgv), dim=1)
    else:
        H_input = torch.cat((secret_imgv,secret_attention1),dim=1)
#        H_input = secret_imgv


    #print(f'........{H_input.shape}')

    #H_input = H_input.reshape(1,3,256,256)
    #H_input = H_input[:,2:5,:,:]
    #print(f'........{H_input.shape}')
#    H_input =dwt(H_input)

    itm_secret_img = Hnet(x=(H_input.cuda()),rev=False)
    output_steg = itm_secret_img.narrow(1, 0, 4 * 3)
    output_z = itm_secret_img.narrow(1, 4 * 3, itm_secret_img.shape[1] - 4 * 3)
#    output_z1 = F.interpolate(output_z, scale_factor=2)
#    output_z1 = iwt(output_z)

    # 将张量拉平成一维数组，并计算像素值的频率分布
#    array = output_z1.cpu().detach().numpy()
#    array = array.astype(np.int64)
 #   freq = np.bincount(array.flatten(), minlength=256)

    # 输出结果
#    for i in range(len(freq)):
#        print(f"Pixel value {i}: {freq[i]}")
    itm_secret_img = iwt(output_steg)

#    itm_secret_img = iwt(itm_secret_img)
    if i_c !=None:
    ...
    if Se_two == 2: 
        ...
        itm_secret_img = itm_secret_img + Se_two*torch.rand(itm_secret_img.size())
    if opt.cover_dependent:
...
#        print(container_img.shape)
    errH = criterion(container_img, cover_imgv)  # Hiding net
    container_img1 =dwt(container_img)
    container_img1 = F.interpolate(container_img1, scale_factor=2)
#    print(container_img1.shape)
#    import numpy as np

    # 定义一个 1x3x256x256 的张量
#    tensor = np.zeros((1, 3, 256, 256), dtype=float)

    # 将张量中的所有元素都设置为 0.5
#    tensor[0, :, :, :] = 0.5

#    tensor = torch.from_numpy(tensor).cuda()
#    output_z_guass= tensor
#    output_z_guass=dwt(output_z_guass)
#    print(output_z_guass.shape)
    output_z_guass = gauss_noise(output_z.shape)
    output_z_guass1 = F.interpolate(output_z_guass, scale_factor=2)
    output_z_guass1= output_z_guass1.to(torch.float32)


    fake_img = Image.open("../fake/clean.png")
    transform = transforms.Compose([transforms.ToTensor()])
    fake_img = transform(fake_img).cuda()

    fake_img = fake_img.view(batch_size_cover, 3, 256, 256)
    fake_template = torch.randn(1, 12, 256, 256).cuda()
    x = torch.cat((container_img1, output_z_guass1), 1)
    y = torch.cat((container_img1,fake_template),1).cuda()
    rev_secret_img = Hnet(x,rev=True).cuda()

    fake_rev_secret_img =Hnet(y,rev=True)

    rev_secret_img1 = F.interpolate(rev_secret_img, scale_factor=0.5)
    fake_rev_secret_img1=F.interpolate(fake_rev_secret_img, scale_factor=0.5)
    rev_secret_img = rev_secret_img1[:, :12, :, :]
    rev_secret_attention = rev_secret_img1[:, 12:, :, :]# 取后三通道
    fake_rev_secret_img =fake_rev_secret_img1[:, :12, :, :]

    fake_rev_secret_attention=fake_rev_secret_img1[:, :12, :, :]
#    print(rev_secret_img.shape)
    rev_secret_img = iwt(rev_secret_img)
    rev_secret_attention=iwt(rev_secret_attention)
    fake_rev_secret_img =iwt(fake_rev_secret_img)
    fake_rev_secret_attention=iwt(fake_rev_secret_attention)
    rev_secret_attention = F.interpolate(rev_secret_attention, scale_factor=0.5)
    rev_secret_attention =secret_attention_model(rev_secret_attention)
    rev_secret_attention = F.interpolate(rev_secret_attention, scale_factor=2)
    fake_rev_secret_attention = F.interpolate(fake_rev_secret_attention, scale_factor=0.5)
    fake_rev_secret_attention = secret_attention_model(fake_rev_secret_attention)
    fake_rev_secret_attention = F.interpolate(fake_rev_secret_attention, scale_factor=2)
#    print(rev_secret_attention.shape)
    errR = criterion(rev_secret_img, secret_imgv_nh)  # Reveal net
    errS = criterion(rev_secret_attention,secret_attention)
    errF = criterion(fake_rev_secret_img,fake_img)
    errS1= criterion(fake_rev_secret_attention,secret_attention)
    errT =criterion(output_z_guass,output_z)
    # L1 metric
    diffH = (container_img-cover_imgv).abs().mean()*255
    diffR = (rev_secret_img-secret_imgv_nh).abs().mean()*255
    return cover_imgv, container_img, secret_imgv_nh, rev_secret_img,fake_rev_secret_img, errH, errR,errS,errF,errS1,errT, diffH, diffR,

def train(train_loader, epoch, Hnet, criterion):
    ...
    for i, ((secret_img, secret_target), (cover_img, cover_target)) in enumerate(train_loader, 0):
        ...
        cover_imgv, container_img, secret_imgv_nh, rev_secret_img, fake_rev_secret_img,errH, errR,errS,errF,errS1,errT, diffH, diffR \
        = forward_pass(secret_img, secret_target, cover_img, cover_target, Hnet, criterion)
        ...
        Slosses.update(errS.item(), opt.bs_secret * opt.num_secret * opt.num_training)
        Flosses.update(errF.item(), opt.bs_secret * opt.num_secret * opt.num_training)
        S1losses.update(errS1.item(), opt.bs_secret * opt.num_secret * opt.num_training)
        Tlosses.update(errF.item(), opt.bs_secret * opt.num_secret * opt.num_training)
        ...
        err_sum = errH + betaerrR_secret+errS+errF+errS1+errT
        optimizer.zero_grad()
        err_sum.backward()
        optimizer.step()
        ...
        log = '[%d/%d][%d/%d]\tLoss_H: %.6f Loss_R: %.6fLoss_S: %.6f Loss_F: %.6fLoss_S1: %.6fLoss_T: %.6fL1_H: %.4f L1_R: %.4f \tdatatime: %.4f \tbatchtime: %.4f' % (
            epoch, opt.epochs, i, opt.iters_per_epoch,
            Hlosses.val, Rlosses.val,Slosses.val,Flosses.val,S1losses.val,Tlosses.val, Hdiff.val, Rdiff.val, data_time.val, batch_time.val)

        if i % opt.logFrequency == 0:
            print(log)

        if epoch <= 0 and i % opt.resultPicFrequency == 0:
            save_result_pic(opt.bs_secret*opt.num_training, cover_imgv, container_img.data, secret_imgv_nh, rev_secret_img.data,fake_rev_secret_img.data, epoch, i, opt.trainpics)
    ...
    epoch_log = "Training[%d] Hloss=%.6f\tRloss=%.6f\tSloss=%.6f\tFloss=%.6f\tS1loss=%.6f\tTloss=%.6f\tHdiff=%.4f\tRdiff=%.4f\tlr= %.6f\t Epoch time= %.4f" % (epoch, Hlosses.avg, Rlosses.avg,Slosses.avg,Flosses.avg,S1losses.avg,Tlosses.avg, Hdiff.avg, Rdiff.avg, optimizer.param_groups[0]['lr'], batch_time.sum)
    ...
    if not opt.debug:
        ...
        writer.add_scalar('train/S_loss', Slosses.avg, epoch)
        writer.add_scalar('train/F_loss', Flosses.avg, epoch)
        writer.add_scalar('train/S1_loss', S1losses.avg, epoch)
        writer.add_scalar('train/T_loss', Tlosses.avg, epoch)
        writer.add_scalar('train/sum_loss', SumLosses.avg, epoch)
        writer.add_scalar('train/H_diff', Hdiff.avg, epoch)
        writer.add_scalar('train/R_diff', Rdiff.avg, epoch)

def validation(val_loader, epoch, Hnet, criterion):
    print(
        "#################################################### validation begin ########################################################")
    start_time = time.time()
    Hnet.eval()
    ...
    for i, ((secret_img, secret_target), (cover_img, cover_target)) in enumerate(val_loader, 0):
        ...
        cover_imgv, container_img, secret_imgv_nh, rev_secret_img,fake_rev_secret_img, errH, errR,errS,errF,errS1,errT, diffH, diffR \
        = forward_pass(secret_img, secret_target, cover_img, cover_target, Hnet,  criterion, val_cover=1)
        ...
        Flosses.update(errF.item(), opt.bs_secret * opt.num_secret * opt.num_training)
        S1losses.update(errS1.item(), opt.bs_secret * opt.num_secret * opt.num_training)
        Tlosses.update(errS1.item(), opt.bs_secret * opt.num_secret * opt.num_training)
        Hdiff.update(diffH.item(), opt.bs_secret * opt.num_cover * opt.num_training)
        Rdiff.update(diffR.item(), opt.bs_secret * opt.num_secret * opt.num_training)

        if i == 0:
            save_result_pic(opt.bs_secret*opt.num_training, cover_imgv, container_img.data, secret_imgv_nh, rev_secret_img.data,fake_rev_secret_img.data, epoch, i, opt.validationpics)
        if opt.num_secret >= 6:
            i_total = 80
        else:
            i_total = 200
        ...
        val_log = "validation[%d] val_Hloss = %.6f\t val_Rloss = %.6f\t val_Sloss = %.6f\tval_Floss = %.6f\tval_S1loss = %.6f\tval_Tloss = %.6f\tval_Hdiff = %.6f\t val_Rdiff=%.2f\t batch time=%.2f" % (
            epoch, Hlosses.val, Rlosses.val,Slosses.val,Flosses.val,S1losses.val,Tlosses.val, Hdiff.val, Rdiff.val, batch_time.val)
        if i % opt.logFrequency == 0:
            print(val_log)

    val_log = "validation[%d] val_Hloss = %.6f\t val_Rloss = %.6f\t val_Sloss = %.6f\tval_Floss = %.6f\tval_S1loss = %.6f\tval_Tloss = %.6f\tval_Hdiff = %.4f\t val_Rdiff=%.4f\t validation time=%.2f" % (
        epoch, Hlosses.avg, Rlosses.avg,Slosses.avg,Flosses.avg,S1losses.avg,Tlosses.avg, Hdiff.avg, Rdiff.avg, batch_time.sum)
...
#    for i in range(N):
 ...
 #   print("Avg. LPIPS S:", lpips.mean().item())
    ...
    ####################################### Cover Agnostic S' #######################################
    cover_imgv, container_img, secret_imgv_nh, rev_secret_img,fake_rev_secret_img, errH, errR,errS,errF,errS1,errT, diffH, diffR \
        = forward_pass(secret_img, secret_target, cover_img.clone().fill_(0.0), cover_target, Hnet, criterion,
                       val_cover=1)
    secret_encoded = container_img - cover_imgv
 ...
 #   lpips = model.forward(secret_imgv_nh, rev_secret_img)
  #  print("Avg. LPIPS S':", lpips.mean().item())
    ...
    if not opt.debug:
        ...
        writer.add_scalar('validation/S_loss_avg', Slosses.avg, epoch)
        writer.add_scalar('validation/F_loss_avg', Flosses.avg, epoch)
        writer.add_scalar('validation/S1_loss_avg', S1losses.avg, epoch)
        writer.add_scalar('validation/T_loss_avg', Tlosses.avg, epoch)
...
def save_result_pic_analysis(bs_secret_times_num_training, cover, container, secret, rev_secret,fake_rev_secret, epoch, i, save_path=None, postname=''):
    ...
    fake_rev_secret = fake_rev_secret[:4]
...
#    secret_gap = (secret_gap*10 + 0.5).clamp_(0.0, 1.0)
    ...
    for i_secret in range(4):
        ...
        fake_rev_secret_i = fake_rev_secret[:,i_secret*opt.channel_secret:(i_secret+1)*opt.channel_secret,:,:]
        secret_gap_i = secret_gap[:,i_secret*opt.channel_secret:(i_secret+1)*opt.channel_secret,:,:]

        if i_secret == 0:
            showSecret = torch.cat((secret_i, rev_secret_i, fake_rev_secret_i,secret_gap_i),0)
        else:
            showSecret = torch.cat((showSecret, secret_i, rev_secret_i,fake_rev_secret_i, secret_gap_i),0)
    ...
    vutils.save_image(showAll, resultImgName, nrow=7, padding=1, normalize=False)

# save result pic and the coverImg filePath and the secretImg filePath
def save_result_pic(bs_secret_times_num_training, cover, container, secret, rev_secret,fake_rev_secret, epoch, i, save_path=None,postname=''):
...
#    resultImgName = '{}/ResultPics_epoch{:03d}_batch{:04d}_{}.png'.format(save_path, epoch, i, postname)
    cover_gap = container - cover
    secret_gap = rev_secret - secret
#    cover_gap = (cover_gap*10 + 0.5).clamp_(0.0, 1.0)
#    secret_gap = (secret_gap*10 + 0.5).clamp_(0.0, 1.0)
